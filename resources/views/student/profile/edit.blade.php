...
<div class="gemini-card" style="max-width: 800px; margin: 0 auto;">
    <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 32px;">
        <a href="{{ route('student.profile') }}" class="gemini-btn-icon">
    ...
    <form method="POST" action="{{ route('student.profile.update') }}" enctype="multipart/form-data">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 32px;">
            <div>
                ...
                <input type="text" name="first_name" class="gemini-input" value="{{ $user->first_name }}" required>
            </div>
            
            <div>
                <label class="gemini-label">Familiya *</label>
                <input type="text" name="last_name" class="gemini-input" value="{{ $user->last_name }}" required>
            </div>
            
            <div>
                <label class="gemini-label">Telefon *</label>
                <input type="text" name="phone" class="gemini-input" value="{{ $user->phone }}" required>
            </div>
            
            <div>
                <label class="gemini-label">Tug'ilgan sana *</label>
                <input type="date" name="birth_date" class="gemini-input" value="{{ $user->birth_date?->format('Y-m-d') }}" required>
            </div>
        </div>
        ...
        <div style="display: flex; justify-content: space-between; gap: 12px;">
            <a href="{{ route('student.profile') }}" class="gemini-btn" style="background: #6c757d;">
